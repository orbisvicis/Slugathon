...
class PickMoveType(Gtk.Dialog):
    ...
    def __init__(self, playername, legion, hexlabel, def1, parent):
        ...
        legion_hbox.pack_start(marker_hbox, expand=True, fill=True, padding=0)

        marker = Marker.Marker(legion, True, scale=20)
        marker_hbox.pack_start(marker.event_box, expand=False, fill=True, padding=0)

        chits_hbox = Gtk.HBox(spacing=3)
        legion_hbox.pack_start(chits_hbox, expand=True, fill=True, padding=0)
        ...
        self.vbox.pack_start(button_hbox, expand=True, fill=True, padding=0)
