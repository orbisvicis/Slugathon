...
from slugathon.gui import (GUIBattleHex, Chit, PickRecruit, SummonAngel,
                           PickCarry, PickStrikePenalty, InfoDialog,
                           ConfirmDialog, Marker, TurnTrack,
...
class GUIBattleMap(Gtk.EventBox):
    ...
    def __init__(self, battlemap, game=None, user=None, playername=None,
        ...
        self.modify_bg(Gtk.StateFlags.NORMAL, Gtkcolor)
        ...
        self.create_ui()
        self.vbox.pack_start(self.ui.get_widget("/Menubar"), expand=False, fill=True, padding=0)
        self.vbox.pack_start(self.ui.get_widget("/Toolbar"), expand=False, fill=True, padding=0)
        self.hbox1 = Gtk.HBox()
        self.hbox2 = Gtk.HBox()
        self.hbox3 = Gtk.HBox()
        self.hbox4 = Gtk.HBox()
        self.hbox5 = Gtk.HBox(homogeneous=True)
        self.vbox.pack_start(self.hbox1, expand=False, fill=True, padding=0)
        self.vbox.pack_start(self.hbox2, expand=True, fill=True, padding=0)
        self.vbox.pack_start(self.hbox3, expand=False, fill=True, padding=0)
        self.vbox.pack_start(self.hbox4, expand=False, fill=True, padding=0)
        self.vbox.pack_start(self.hbox5, expand=False, fill=True, padding=0)
        if game:
            self.vbox.pack_start(event_log, expand=True, fill=True, padding=0)
            Gtkcolor = Gtk.gdk.color_parse("white")
            attacker_marker = Marker.Marker(game.attacker_legion, True,
                                            self.scale / 2)
            attacker_marker.event_box.modify_bg(Gtk.StateFlags.NORMAL, Gtkcolor)
            defender_marker = Marker.Marker(game.defender_legion, True,
                                            self.scale / 2)
            defender_marker.event_box.modify_bg(Gtk.StateFlags.NORMAL, Gtkcolor)
            board = game.board
            ...
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(own_hex_label, expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox1.pack_start(self.spacer(), expand=True, fill=True, padding=0)
            self.hbox3.pack_start(self.turn_track, expand=False, fill=True, padding=0)
            self.hbox3.pack_start(self.battle_dice, expand=True, fill=True, padding=0)
            self.hbox2.pack_start(attacker_marker.event_box, expand=True, fill=True, padding=0)
        self.hbox2.pack_start(self.area, expand=True, fill=True, padding=0)
        if game:
            self.hbox2.pack_start(defender_marker.event_box, expand=True, fill=True, padding=0)
            self.hbox5.pack_start(self.attacker_graveyard, expand=True, fill=True, padding=0)
            self.hbox5.pack_start(self.defender_graveyard, expand=True, fill=True, padding=0)
    ...
    def spacer(self):
        ...
        eventbox = Gtk.EventBox()
        Gtkcolor = Gtk.gdk.color_parse("white")
        eventbox.modify_bg(Gtk.StateFlags.NORMAL, Gtkcolor)
        return eventbox
    ...
    def clicked_on_background(self, area, event):
        ...
        if not self.game or not self.game.battle_active_player:
            return
        if self.game.battle_phase is Phase.PhaseBattle.MANEUVER:
            if self.game.battle_active_player.name == self.playername:
                self.highlight_mobile_chits()
        elif (self.game.battle_phase is Phase.PhaseBattle.STRIKE
              or self.game.battle_phase is Phase.PhaseBattle.COUNTERSTRIKE):
            if self.game.battle_active_player.name == self.playername:
    ...
    def clicked_on_hex(self, area, event, guihex):
        ...
        if phase is Phase.PhaseBattle.MANEUVER:
        ...
        elif phase is Phase.PhaseBattle.STRIKE or phase is Phase.PhaseBattle.COUNTERSTRIKE:
            self.highlight_strikers()
    ...
    def _add_missing_chits(self):
        ...
        for (legion, rotate) in \
            ...
            , (self.game.defender_legion, GdkPixbuf.PixbufRotation.COUNTERCLOCKWISE)
            ]:
            if legion:
    ...
    def cb_concede2(self, confirmed):
        ...
        if confirmed:
            ...
            def1 = self.user.callRemote("concede", self.game.name,
                                        friend.markerid, enemy.markerid,
                                        friend.hexlabel)
            def1.addErrback(self.failure)
    ...
    def update(self, observed, action, names):
        ...
        if (isinstance(action, Action.MoveCreature) or
                isinstance(action, Action.UndoMoveCreature)):
            ...
            if (action.carries and self.game.battle_active_player and
               self.game.battle_active_player.name == self.playername):
                ...
                strikers = self.game.battle_active_legion.strikers
                if not strikers:
                    if self.game.battle_phase is Phase.PhaseBattle.STRIKE:
                        def1 = self.user.callRemote("done_with_strikes",
                                                    self.game.name)
                        def1.addErrback(self.failure)
                    else:
    ...
    def picked_reinforcement(self, result):
        legion, creature, recruiter_names = result
    ...
    def picked_carry(self, result):
        carry_target, carries = result
        logging.info("picked_carry %s %s", carry_target, carries)
        if self.pickcarry is not None:
            self.pickcarry.destroy()
        self.pickcarry = None
        def1 = self.user.callRemote("carry", self.game.name,
                                    carry_target.name, carry_target.hexlabel,
                                    carries)
        def1.addErrback(self.failure)

    def picked_strike_penalty(self, result):
        striker, target, num_dice, strike_number = result
