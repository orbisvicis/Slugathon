...
class Lobby(Gtk.EventBox):
    ...
    def __init__(self, user, playername, playernames, games, parent_window):
        ...
        vbox1.pack_start(hbox1, expand=False, fill=True, padding=0)

        new_game_button = Gtk.Button()
        hbox1.pack_start(new_game_button, expand=False, fill=True, padding=0)
        ...
        hbox2.pack_start(image1, expand=False, fill=True, padding=0)
        label1 = Gtk.Label("Start new game")
        hbox2.pack_start(label1, expand=False, fill=True, padding=0)

        load_game_button = Gtk.Button()
        # Disable it until loading games works.
        load_game_button.set_sensitive(False)
        hbox1.pack_start(load_game_button, expand=False, fill=True, padding=0)
        ...
        hbox3.pack_start(image2, expand=False, fill=True, padding=0)
        label2 = Gtk.Label("Load saved game")
        hbox3.pack_start(label2, expand=False, fill=True, padding=0)

        hbox4 = Gtk.HBox()
        vbox1.pack_start(hbox4, expand=True, fill=True, padding=0)

        vbox2 = Gtk.VBox()
        hbox4.pack_start(vbox2, expand=True, fill=True, padding=0)

        label3 = Gtk.Label("Chat messages")
        vbox2.pack_start(label3, expand=False, fill=True, padding=0)
        ...
        vbox2.pack_start(label4, expand=False, fill=True, padding=0)

        self.chat_entry = Gtk.Entry(max_length=80)
        self.chat_entry.set_width_chars(80)
        vbox2.pack_start(self.chat_entry, expand=False, fill=True, padding=0)

        scrolled_window3 = Gtk.ScrolledWindow()
        scrolled_window3.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        hbox4.pack_start(scrolled_window3, expand=True, fill=True, padding=0)
    ...
    def _add_wfp(self, game):
        ...
        if wfp is None:
            ...
            wfp.connect("destroy", self.cb_wfp_destroy)
    ...
    def cb_user_list_select(self, selection, model, path, is_selected, user_data):
        index = path[0]
    ...
    def cb_game_list_select(self, selection, model, path, is_selected, user_data):
        index = path[0]
